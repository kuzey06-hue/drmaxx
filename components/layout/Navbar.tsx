...
export function Navbar() {
  ...
  const handleUserLeave = () => { userTimer.current = setTimeout(() => setUserOpen(false), 150); };
  ...
  const handleCatLeave = () => {
    leaveTimer.current = setTimeout(() => setCatOpen(false), 120);
  };
          ...
          <div className="flex h-16 items-center gap-6">
            ...
            <div className="hidden md:flex flex-1 max-w-xs relative">
              <input
                ...
                className="w-full h-9 pl-9 pr-4 rounded-xl bg-white/10 border border-white/10 text-white text-sm placeholder:text-white/40 outline-none focus:border-orange-500/60 focus:bg-white/15 transition-all"
              />
                ...
                <AnimatePresence>
                  {userOpen && user && (
                    <motion.div
                      ...
                      className="absolute right-0 top-full pt-2 z-50 w-52"
                      onMouseEnter={handleUserEnter}
                      onMouseLeave={handleUserLeave}
                    >
