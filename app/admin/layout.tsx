...
const navGroups: { title?: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Orijinallik QR", href: "/admin/orijinallik", icon: ShieldCheck },
      { label: "Affiliate", href: "/admin/affiliate", icon: Megaphone },
...
const pageTitles: Record<string, string> = {
  ...
  "/admin/orijinallik": "Orijinallik QR Yönetimi",
  "/admin/affiliate": "Affiliate Yönetimi",
...
function NavItemComponent({ item, pathname }: { item: NavItem; pathname: string }) {
      ...
      <div>
        ...
        >
          <Icon
            size={17}
            className={
              isActive ? "text-orange-400" : "text-white/40 group-hover:text-white/80"
            }
          />
  ...
  return (
    ...
    >
      <Icon
        size={17}
        className={isActive ? "text-white" : "text-white/40 group-hover:text-white/80"}
      />
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const pageTitle =
    pageTitles[pathname] ??
    (pathname.startsWith("/admin/icerik/blog/duzenle")
  ...
  const fetchNotifications = async () => {
    const res = await fetch("/api/cms/notifications", { cache: "no-store" }).catch(() => null);
      ...
      <aside className="w-64 flex-shrink-0 bg-[#0F1729] flex flex-col">
        ...
        <div className="px-3 py-4 border-t border-white/10 space-y-1">
          ...
          >
            ...
            <LogOut
              size={14}
              className="text-white/30 group-hover:text-red-400 flex-shrink-0 transition-colors"
            />
          </button>
        </div>
      </aside>
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <button
                ...
                className="relative w-9 h-9 rounded-xl border border-gray-100 hover:bg-gray-50 flex items-center justify-center text-gray-500"
              >
                <Bell size={16} />
                {unreadCount > 0 && (
              ...
              {notifOpen && (
                <div className="absolute right-0 top-11 w-80 bg-white border border-gray-100 shadow-xl rounded-2xl z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    <p className="text-sm font-bold text-gray-900">Bildirimler</p>
                    <button onClick={markAllRead} className="text-[11px] font-semibold text-orange-500">
                      Okundu yap
                    </button>
                  </div>
                  <div className="max-h-96 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <p className="p-4 text-sm text-gray-400">Bildirim yok.</p>
                    ) : (
                      notifications.slice(0, 10).map((item) => (
                        ...
                        >
                          <div className="flex gap-2">
                            {!item.read && <span className="mt-1.5 w-2 h-2 rounded-full bg-orange-500 flex-shrink-0" />}
