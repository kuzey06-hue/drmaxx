...
export default function OdemePage() {
  ...
  const applyKupon = async (codeToApply: string) => {
    ...
    const res = await fetch("/api/cms/kuponlar/validate", {
      ...
      body: JSON.stringify({ code: codeToApply.trim(), orderTotal: subtotal + kargo }),
    });
  ...
  useEffect(() => {
    ...
    const fromQuery = params.get("kupon");
    const fromStorage = localStorage.getItem("drmaxx_coupon_code");
    const initial = (fromQuery || fromStorage || "").trim().toUpperCase();
    if (!initial || kuponOK) return;
    setKuponKod(initial);
    void applyKupon(initial);
  }, [kuponOK, subtotal, kargo]);
  ...
  const handleDemoOrder = async () => {
    ...
    await fetch("/api/cms/orders", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ orderNo: currentOrderNo, status: "İşleniyor", paymentType: "Demo" }),
    });
            ...
            <div className="space-y-6">
              ...
              {step === 2 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div
                    className={`px-6 pt-5 pb-1 grid gap-3 ${
                      bankaActive && bankAccounts.length > 0 ? "grid-cols-2" : "grid-cols-1"
                    }`}
                  >
                    <button
                      ...
                      {paytrToken && (
                        <div className="p-2 sm:p-4">
                          ...
                          <iframe
                            ...
                            frameBorder="0"
                            scrolling="yes"
                            style={{ width: "1px", minWidth: "100%", height: "680px" }}
                            onLoad={initPaytrIframe}
                          />
                        </div>
                      )}
