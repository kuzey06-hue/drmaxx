...
export async function POST(req: NextRequest) {
  ...
  const row: Record<string, unknown> = { id: body.id || String(Date.now()), slug, ...toDb(body) };
  const { data, error } = await supabaseAdmin.from("products").insert(row).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (Number(row.stock ?? 0) <= 10) {
    await createAdminNotification({
      type: "low_stock",
      title: "Düşük stok uyarısı",
      message: `${row.name ?? "Ürün"} stok: ${row.stock ?? 0}`,
      href: "/admin/urunler",
    }).catch(() => {});
  }
...
export async function PUT(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (row.stock !== undefined && Number(row.stock) <= 10) {
    await createAdminNotification({
      type: "low_stock",
      title: "Düşük stok uyarısı",
      message: `${rest.name ?? "Ürün"} stok: ${row.stock}`,
      href: "/admin/urunler",
    }).catch(() => {});
  }
