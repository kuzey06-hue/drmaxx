...
// POST /api/cms/orders — yeni sipariş
export async function POST(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ...data, orderNo: data.order_no }, { status: 201 });
}

// PUT /api/cms/orders — durum güncelle (id ile)
...
// PATCH /api/cms/orders — durum güncelle (orderNo ile, PayTR callback)
