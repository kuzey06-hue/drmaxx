...
function GirisForm() {
  ...
  const [mode, setMode] = useState<"giris" | "kayit">(
    modeParam === "kayit" ? "kayit" : "giris",
  );
  ...
  useEffect(() => {
    if (modeParam === "kayit") setMode("kayit");
    if (modeParam === "giris") setMode("giris");
  }, [modeParam]);
  ...
  const handleGoogle = async () => {
    if (!googleEnabled) {
      setError("Google girişi yapılandırılmamış. Lütfen yöneticiye bildirin.");
      return;
    }
